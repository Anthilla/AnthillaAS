...
namespace AnthillaCore.Mail {
    ...
    public class Anth_MailRepository {
        ...
        public Anth_MailModel Create2(string guid, string from, List<string> to, string body, string sub, string box, string tags) {
            Anth_MailModel model;
            var m = DeNSo.Session.New.Get<Anth_MailModel>(mm => mm.strMailSubject == sub).FirstOrDefault();
            if (m != null) {
                model = m;
                model.strMailFrom = AnthillaSecurity.Encrypt(from, model.StorIndexN2, model.StorIndexN1);
                var toTab = new List<byte[]>();
                foreach (var t in to) {
                    var tod = AnthillaSecurity.Encrypt(t, model.StorIndexN2, model.StorIndexN1);
                    toTab.Add(tod);
                }
                model.strMailTo = toTab;
                model.strMailBody = AnthillaSecurity.Encrypt(body, model.StorIndexN2, model.StorIndexN1);
        ...
        public Anth_MailModel Import(MailMessage mail, string guid, string tags) {
            var model = new Anth_MailModel();
            model.ADt = DateTime.Now.ToString();
            model.Aned = "n";
            model.ARelGuid = Guid.NewGuid().ToString().Substring(0, 8);
        ...
        public Anth_MailModel Edit(string id, string from, string to, string body, string sub, string box, string tags) {
            ...
            model.MailboxId = box;
            model.Tags = tag.Tagger(tags).ToList();
            model.isRead = oldItem.isRead;
            model.MessageType = oldItem.MessageType;
        ...
        public Anth_MailModel AssignCompany(string objId, string compguid) {
            Anth_MailModel user = DeNSo.Session.New.Get<Anth_MailModel>(model => model.MailGuid == objId && model.IsDeleted == false).FirstOrDefault();
            user.CompanyIds.Add(compguid);
            DeNSo.Session.New.Set(user);
            return user;
        }

        public Anth_MailModel AssignProject(string objId, string projGuid) {
            Anth_MailModel user = DeNSo.Session.New.Get<Anth_MailModel>(model => model.MailGuid == objId && model.IsDeleted == false).FirstOrDefault();
