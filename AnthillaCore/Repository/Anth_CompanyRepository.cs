...
namespace AnthillaCore.Repository {
    ...
    public class Anth_CompanyRepository {
        ...
        public IEnumerable<Anth_Dump> GetAllByUsers(string[] users) {
            var table = new List<Anth_Dump>();
            var guidList = new List<string>();
            foreach (string userGuid in users) {
                var user = userRepo.GetById(userGuid);
                var compGuid = user.AnthillaCompanyId;
                if (!guidList.Contains(compGuid)) {
                    guidList.Add(compGuid);
                }
            }
            ...
            Anth_Log.TraceEvent("Company Management", "Information", "tmp", "Company get all by user(s)");
            return table;
        }
        ...
        public Anth_CompanyModel Create(string guid, string alias, string tags) {
            ...
            model.CompanyOwner = 0;

            var naming = Anth_Naming.GetByType("company");
            if (naming != null) {
                var na = Anth_NamingActions.SetCompanyAlias(alias);
                model.CompanyAlias = AnthillaSecurity.Encrypt(na, model.StorIndexN2, model.StorIndexN1);
            }
            else {
